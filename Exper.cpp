...
bool search_reverse_matrix(vector <vector<double>> &matrix, int num_threads)
{
...
int main() {
    ...
    for (int num_threads = 1; num_threads <= 8; num_threads++) {
        omp_set_num_threads(num_threads);
            ...
            if (!search_reverse_matrix(matrix, num_threads)) {
            ...
            std::cout << "Время выполнения: " << duration.count() / 10 << " мс." << " Потоки: " << num_threads
                      << " Применение Schedule : Все циклы" << std::endl;
    }
    return 0;
}